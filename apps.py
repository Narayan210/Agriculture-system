...
fig4 = px.histogram(df, x="Rainfall_mm", y="Yield_kg_per_hectare", title='🌧 Rainfall vs Yield')
...
fig_sunny = px.line(avg_yield_sunny, x="Sunny_Days_Cat", y="Yield_kg_per_hectare", markers=True,
                    title="☀ Average Yield Based on Sunny Days Category")
...
# Display results
st.write(f"☀ *Total Sunny Days over {duration} months: {total_sunny_days}*")
st.write(f"🌧 *Total Rainfall over {duration} months: {total_rainfall} mm*")
